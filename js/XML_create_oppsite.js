(function(root, UMDCP) {
  if (typeof define === "function" && define.amd) define([], UMDCP);
  else if (typeof module === "object" && module.exports)
    module.exports = UMDCP();
  else root.xmlRequestCreateOpposite = UMDCP();
})(this, function UMDCP() {
  function XMLP() {
    var xmlRequestCreateOpposite = {
      ...
      page: function(XML, DOMElement) {
        ...
        self.defineHeightParameterElement = 197;
        ...
        if (checkType !== undefined) type = checkType.type;
      ...
      element: function(object) {
          ...
          on: {
            click: function() {
              ...
              this._updateTimeOut = undefined;
                ...
                function() {
                    self.onchangeTabPoint(childTrueDame);
                }.bind(this),
                500
                );
            },
          ...
          on: {
            click: function() {
              if (this._updateTimeOut !== undefined) {
                ...
                this._updateTimeOut = setTimeout(
                function() {
                    self.onchangeTabPointBack(childPointDame);
                }.bind(this),
                500
                );
            },
        ...
        temp.getValue = function() {
          ...
          if (temp.mode === 0) {
            ...
            result =
              ...
              object.tagName +
              ">" +
              temp.questionPoint.getValue() +
              temp.answerPoint.getValue() +
              "</" +
              object.tagName +
              ">";
          }
        ...
        temp.unfold_less = function() {
          ...
          temp.cloneSortQuestion = self.cloneSortQuestion(
            absol.XML.parse(temp.question.getValue()),
            temp
          );
        };
        temp.unfold_more = function() {
      ...
      cloneSortQuestion: function(object, childContainer) {
        ...
        var temp = absol.buildDom({
          tag: "div",
          class: "freebirdFormeditorViewItemContent",
          child: [scrollerTab, textValue],
          on: {
            click: function() {
              var el = this;
              while (
                !el.classList.contains("freebirdFormeditorViewPagePageCard")
              ) {
                el = el.parentNode;
                if (
                  ...
                  setTimeout(function() {
                    ...
                    var offsetPosition =
                      elementPosition - self.defineHeightHeader;
                    window.scrollTo({
                      top: offsetPosition,
                      behavior: "smooth"
                    });
      ...
      question: function(object, childContainer, tempObject, point, type) {
        ...
        temp.footerElement = this.footerElement(object, importantType, point);
        ...
        temp.getValue = function() {
          var result = "<question>";
          if (temp.header.id != "") result += "<id>" + temp.header.id + "</id>";
          else {
            temp.header.id = ("text_" + Math.random() + Math.random()).replace(
              /\./g,
              ""
            );
          ...
          result += temp.footerElement.getValue();
          if (
            temp.header.sum != "" &&
            temp.header.sum !== "undefined" &&
            temp.header.sum !== undefined
          )
        ...
        temp.setObject = function(object) {
          ...
          if (sum != undefined && sum != "undefined") temp.header.sum = sum;
          else temp.header.sum = 0;
          temp.footerElement.setObject(sum);
        };
      ...
      questionPoint: function(object, childContainer, tempParent, point = 0) {
        ...
        if (tempParent.questionPoint !== undefined) {
          tempParent.questionPoint.question.parentNode.replaceChild(
            x,
            tempParent.questionPoint.question
          );
        } else {
          var scrollerTab = xmlComponent.holdmoveVer();
          childContainer.appendChild(scrollerTab);

          questionPoint.footerElement = self.footerElementPoint();

          childContainer.appendChild(questionPoint.footerElement);
      ...
      answer: function(
        ...
        result.requestUpdateSize = function() {
          if (temp.requestUpdateSize !== undefined) temp.requestUpdateSize();
        };
        return result;
      },
      answerPoint: function(object, childContainer, tempParent) {
        ...
        if (tempParent.answerPoint !== undefined) {
          tempParent.answerPoint.parentNode.replaceChild(
            result,
            tempParent.answerPoint
          );
        } else childContainer.appendChild(result);
      ...
      onchangeTabPoint: function(el) {
        ...
        for (var i = 0; i < el.object.childNodes.length; i++) {
          if (el.object.childNodes[i].tagName === "question") {
            if(el.questionPoint.getValue()!==absol.XML.stringify(el.object.childNodes[i]))
            this.questionPoint(
              el.object.childNodes[i],
              el.childContainer,
              el,
              el.question.header.sum
            );
          }
          if (el.object.childNodes[i].tagName === "answer") {
            if(el.answerPoint.getValue()!==absol.XML.stringify(el.object.childNodes[i]))
      ...
      onchangeTabPointBack: function(el) {
        ...
        el.setObject(
          absol.XML.parse(
            "<test>" +
              el.questionPoint.getValue() +
              el.answerPoint.getValue() +
              "</test>"
          )
        );
      ...
      updateSumPoint: function() {
        ...
        for (var index = 0; index < self.arrPage.length; index++)
          for (
            ...
            if (
              self.arrPage[index].arrElement.childNodes[i].questionPoint
                .question.input !== undefined
            ) {
              point = self.arrPage[index].arrElement.childNodes[
                i
              ].questionPoint.question.input.getValue();
              if (point !== undefined)
                self.sumPoint.childNodes[0].innerHTML =
                  Number(self.sumPoint.childNodes[0].innerHTML) + Number(point);
            }
      ...
      functionScroller: function(listButton) {
        ...
        if (temp1.length !== 0 && elementInViewport(temp1[0])) {
          ...
          this.listButton.getBoundingClientRect().y - self.defineHeightParameterElement <
          self.defineHeightHeader + 40
        )
          this.listButton.style.top = self.modal.scrollTop - 0 + "px";
        else if (
          this.listButton.getBoundingClientRect().y - self.defineHeightParameterElement >
          self.modal.clientHeight - this.listButton.clientHeight
        )
          this.listButton.style.top =
            self.modal.scrollTop +
            self.modal.clientHeight -
            this.listButton.clientHeight -
            (self.defineHeightHeader + 10) +
            "px";
      },
    ...
    return xmlRequestCreateOpposite;
  }
