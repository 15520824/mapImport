...
/*
php:
    function EncodingClass::write_script();
    function EncodingClass::inputvalue();
    function EncodingClass::toVariable($str);
    function EncodingClass::fromVariable($obj);
    function EncodingClass::duplicate($obj);
    function EncodingClass::ordutf8($string, &$offset);
javascript:
    function EncodingClass.md5.encode(str)
    function EncodingClass.utf8.encode(str)
    function EncodingClass.utf8.decode(str)
    function EncodingClass.base64.encode(str)
    function EncodingClass.base64.decode(str)
    function EncodingClass.quickEncrypt(str)
    function EncodingClass.quickDecrypt(str)
    function EncodingClass.inputvalue(str)
    function EncodingClass.textshow(str)
    function EncodingClass.string.addslashes(str)
    function EncodingClass.string.readln(str) // return {result: "first line without CRLF", remain: "next lines"}
    function EncodingClass.string.readlnInt(str)
    function EncodingClass.string.readlnHex(str)
    function EncodingClass.string.readlnFloat(str)
    function EncodingClass.string.toHex(number, [optional] width) // if width < 0, return exactly last -width character (padding 0 if needed)
    function EncodingClass.string.fromHex(str)
    function EncodingClass.string.fromDouble(value, precise)
    function EncodingClass.string.fromVariable(obj)
    function EncodingClass.string.toVariable(str)
    function EncodingClass.string.duplicate(obj)
    function EncodingClass.string.merge(strArray)
*/

$encoding_script_written = 0;
class EncodingClass {

    public static function write_script() {
        global $encoding_script_written;
        if ($encoding_script_written == 1) return 0;
        ?>
    ...
    public static function inputvalue($str) {
        $str = $str."";
		$st = "";
		if (strlen($str) > 0) {
			$offset = 0;
			while ($offset != -1) {
				$z = EncodingClass::ordutf8($str, $offset);
				$st .= "&#".$z.";";
			}
		}
		return $st;
    ...
    public static function toVariable($str) {
        ...
        if (strcmp($st, "date") == 0) {
            ...
            $str = substr($str, $k + 1);
            $r = new DateTime();
            $r->setTimestamp(intval((intval(base64_decode($str)) / 1000).""));
            return $r;
        }
        if (strcmp($st, "number") == 0) {
            $k = strpos($str, "#");
            if ($k === false) return null;
            $sx = substr($str, 0, $k);
            $str = substr($str, $k + 1);
            return floatval(base64_decode($str));
        }
        ...
        if (strcmp($st, "array") == 0) {
            $k = strpos($str, "#");
            if ($k === false) return null;
            $n = intval(substr($str, 0, $k));
            $str = substr($str, $k + 1);
            for ($i = 0; $i < $n; $i++) {
                $k = strpos($str, "#");
                if ($k === false) return null;
                $z = intval(substr($str, 0, $k));
                $x = substr($str, $k+1, $z);
                $str = substr($str, $k + $z + 1);
                $r[$i] = EncodingClass::toVariable($x);
            }
            if (isset($r)) return $r;
            return array();
        }
        if (strcmp($st, "obj") == 0) {
            ...
            for ($i = 0; $i < $n; $i++) {
                ...
                $x = substr($str, $k+1, $z);
                $str = substr($str, $k + $z + 1);
                $r[$kv] = EncodingClass::toVariable($x);
            }
            if (isset($r)) return $r;
            return array();
        }
    ...
    public static function mergeStringArray ($strArray) {
        while (TRUE) {
            $k = sizeof($strArray);
            if ($k > 1) {
                $x = array();
    ...
    public static function fromVariable ($obj) {
        if (is_array($obj)) {
            $n = count($obj);
            $isca = TRUE;
            for ($i = 0; $i < $n; $i++) {
            ...
            if ($isca) {
                $st = "array#".$n."#";
                for ($i = 0; $i < $n; $i++) {
                    $sx = EncodingClass::fromVariable($obj[$i]);
                    array_push($sb, strlen($sx)."#".$sx);
                }
            }
            else {  // object
                $st = "obj#".$n."#";
                foreach ($obj as $key => $value) {
                    $sx = base64_encode($key);
                    array_push($sb, strlen($sx)."#".$sx);
                    $sx = EncodingClass::fromVariable($value);
                    array_push($sb, strlen($sx)."#".$sx);
                }
            }
            return $st.EncodingClass::mergeStringArray($sb);
        }
        else if (is_bool($obj)) {
            if ($obj) {
                return "bool#1";
            ...
            $obj *= 1000;
            $sx = base64_encode($obj."");
            return "date#".strlen($sx)."#".$sx;
        }
        else if (is_int($obj)) {
            $sx = base64_encode($obj."");
            return "number#".strlen($sx)."#".$sx;
        }
    ...
    public static function duplicate ($obj) {
        if (is_array($obj)) {
            ...
            if ($isca) {
                for ($i = 0; $i < $n; $i++) {
                    array_push($t, EncodingClass::duplicate($obj[$i]));
                }
            }
            else {  // object
                foreach ($obj as $key => $value) {
                    $t[$key] = EncodingClass::duplicate($value);
                }
            }
            return $t;
        }
        ...
        if ($obj instanceof DateTime) {
            $x = new DateTime();
            $x->setTimestamp($obj->getTimestamp());
            return $x;
        }
        if (is_int($obj) || is_float($obj) || is_string($obj)) {
            return $obj;
        }
        if ($obj === null) return null;
        return $obj;
    ...
    public static function jsValue ($obj) {
        if (is_array($obj)) {
            $n = count($obj);
            $isca = TRUE;
            for ($i = 0; $i < $n; $i++) {
            ...
            if ($isca) {
                ...
                for ($i = 0; $i < $n; $i++) {
                    if ($i > 0) $st .= ", ";
                    $sx = EncodingClass::jsValue($obj[$i]);
                }
                $st .= "]";
            }
            else {  // object
                ...
                foreach ($obj as $key => $value) {
                    ...
                    $st .= ": ".EncodingClass::jsValue($value);
                    $c++;
                }
                $st .= "}";
            }
    ...
    public static function echo_x($str) {
        $x = strlen($str);
        for ($i = 0; $i < $x; $i += 50) {
            $sx = substr($str, $i, 50);
            echo $sx;
